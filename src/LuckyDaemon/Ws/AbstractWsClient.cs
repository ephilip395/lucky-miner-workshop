    ...
    public abstract class AbstractWsClient : IWsClient
    {
            ...
            get
            {
                switch (_appType)
                {
                    case LuckyAppType.MinerTweak:
                        return LuckyRegistry.GetIsOuterUserEnabled();
        ...
        private string OuterUserId
        {
            get
            {
                ...
                {
                    case LuckyAppType.MinerTweak:
                        return LuckyRegistry.GetOuterUserId();
                    case LuckyAppType.MinerMonitor:
        ...
        // MinerTweak时_outerUserId是来自注册表的OuterUserId，MinerMonitor时_outerUserId是RpcRoot.RpcUser.LoginName
        ...
        #region ctor
        private readonly LuckyAppType _appType;
        public AbstractWsClient(LuckyAppType appType)
        {
            _appType = appType;
            _clientId = LuckyRegistry.GetClientId(appType);
                ...
                if (_ws == null)
                {
                    lock (_wsLocker)
                    {
                        if (_ws == null)
                        {
                            ...
                            var ws = new WebSocket($"{LuckyKeyword.OfficialWsServiceUrlBase}/{_appType.GetName()}")
                            //var ws = new WebSocket($"ws://{server}/{_appType.GetName()}")
                            {
                                Compression = CompressionMethod.Deflate
                            };
                            ws.Log.File = System.IO.Path.Combine(TempPath.TempLogsDirFullName, string.Format(LuckyKeyword.WebSocketSharpLogFileNameFormat, _appType.GetName()));
        ...
        {
            LuckyConsole.DevError(e.Message);
        }
        ...
        private void Ws_OnMessage(object sender, MessageEventArgs e)
        {
            ...
            {// 此时一定IsText，因为取值只有IsBinary、IsPing、IsText这三种
            ...
            {
                LuckyConsole.DevWarn(() => $"OnMessage Received InvalidType {e.Data}");
            }
            #endregion
        }
        ...
        private void ConnectAsync(WebSocket ws)
        {
            ...
            if (_appType == LuckyAppType.MinerMonitor)
            {
