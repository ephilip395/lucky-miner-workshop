...
namespace Lucky {
    public class Cleaner {
        ...
        private void ClearDownload() {
            try {
                foreach (var fileFullName in Directory.GetFiles(MinerTweakTempPath.DownloadDirFullName)) {
        ...
        private void ClearPackages() {
            ...
            foreach (var kernel in LuckyContext.Instance.ServerContext.KernelSet.AsEnumerable().ToArray()) {
        ...
        private void CleanKernels() {
            try {
                foreach (var kernelProcessName in LuckyContext.Instance.ServerContext.KernelSet.GetAllKernelProcessNames()) {
                    if (!LuckyContext.Instance.IsMining || LuckyContext.Instance.LockedMineContext.Kernel.GetProcessName() != kernelProcessName) {
                        Windows.TaskKill.Kill(kernelProcessName, waitForExit: true);
                    }
                }
                string currentKernelDir = string.Empty;
                var currentMineContext = LuckyContext.Instance.LockedMineContext;
                if (currentMineContext != null && currentMineContext.Kernel != null) {
                    currentKernelDir = currentMineContext.Kernel.GetKernelDirFullName();
                }
                foreach (string dir in Directory.GetDirectories(MinerTweakTempPath.KernelsDirFullName)) {
