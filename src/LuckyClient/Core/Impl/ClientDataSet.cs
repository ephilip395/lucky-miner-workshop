...
namespace Lucky.Core.Impl
{
    public class ClientDataSet : ClientDataSetBase, IClientDataSet
    {
        ...
        {
            _ = VirtualRoot.BuildOncePath<ClientSetInitedEvent>("开始拉取矿机算力的进程", LogEnum.None, PathId.Empty, GetType(), PathPriority.Normal, path: message =>
              {
                  PullSpeedInit();
              });
        }
        ...
        public void AddClient(string minerIp)
        {
            ...
            ClientData clientData = ClientData.Create(minerIp, out MinerData minerData);
            _ = _dicByObjectId.TryAdd(clientData.Id, clientData);
        ...
        private void PullSpeedInit()
        {
            ...
            VirtualRoot.BuildEventPath<Per10SecondEvent>("周期拉取矿机状态", LogEnum.DevConsole, this.GetType(), PathPriority.Normal, message =>
            {
                ...
                {
                    LuckyConsole.DevDebug("周期拉取数据更新拍照源数据");
        ...
        private Task CreatePullTask(ClientData clientData)
        {
            ...
            {
                RpcRoot.Client.MinerTweakService.GetSpeedAsync(clientData, (speedData, exception) =>
                {
                    if (exception != null)
                    {
                        ...
                        clientData.MainCoinSpeed = 0;
                        clientData.DualCoinSpeed = 0;
                        foreach (Gpus.GpuSpeedData item in clientData.GpuTable)
                        {
                            item.MainCoinSpeed = 0;
                            item.DualCoinSpeed = 0;
                        }
                    ...
                    else
                    {
                        if (speedData.ClientId != clientData.ClientId)
                        {
                            ...
                            {
                                _ = _dicByClientId.TryRemove(clientData.ClientId, out _);
                            }
