...
namespace Lucky.Core.Profiles.Impl
{
    public class GpuProfileSet : SetBase, IGpuProfileSet
    {
        ...
        public GpuProfileSet(ILuckyContext luckycmContext)
        {
            ...
            VirtualRoot.BuildCmdPath<CoinOverClockCommand>(location: this.GetType(), LogEnum.DevConsole, path: message =>
            {
                ...
                {
                    CoinOverClock(luckycmContext, message.CoinId);
            ...
            if (!string.IsNullOrEmpty(json))
            {
                ...
                if (data != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            VirtualRoot.MyLocalWarn(nameof(GpuProfileSet), "检测到本机显卡发生过变更，请重新填写超频数据。", OutEnum.Warn);
                        }
                        Save();
                    }
                }
        ...
        public void RemoteOverClock()
        {
            ...
            VirtualRoot.Execute(new CoinOverClockCommand(LuckyContext.Instance.MinerProfile.CoinId));
        }
        ...
        private void CoinOverClock(ILuckyContext luckycmContext, Guid coinId)
        {
            try
            {
                if (IsOverClockGpuAll(coinId))
                {
                    GpuProfileData overClockData = _data.GpuProfiles.FirstOrDefault(a => a.CoinId == coinId && a.Index == LuckyContext.GpuAllId);
                    if (overClockData != null)
                    {
                        OverClock(luckycmContext, overClockData);
                    }
                }
                else
                {
                    ...
                    {
                        if (overClockData.Index != LuckyContext.GpuAllId)
                        {
                            OverClock(luckycmContext, overClockData);
                        }
                    }
        ...
        private void OverClock(ILuckyContext luckycmContext, IGpuProfile data)
        {
#if DEBUG
            NTStopwatch.Start();
#endif
            if (luckycmContext.GpuSet.TryGetGpu(data.Index, out IGpu gpu))
            {
                luckycmContext.GpuSet.OverClock.OverClock(gpuIndex: data.Index, OverClockValue.Create(data));
                if (data.Index == LuckyContext.GpuAllId)
                {
                    LuckyConsole.UserOk($"统一超频：{data.ToOverClockString()}");
                }
                else
                {
                    LuckyConsole.UserOk($"GPU{gpu.Index}超频：{data.ToOverClockString()}");
                }
                2.SecondsDelay().ContinueWith(t =>
                {
                    luckycmContext.GpuSet.OverClock.RefreshGpuState(data.Index);
                });
            }
#if DEBUG
            ...
            {
                LuckyConsole.DevTimeSpan($"耗时{elapsedMilliseconds} {this.GetType().Name}.{nameof(OverClock)}");
            }
#endif
        }
        ...
        private void Save()
        {
            _data.GpuType = LuckyContext.Instance.GpuSet.GpuType;
