    ...
    internal class AMDGpuSet : IGpuSet {
        ...
        public AMDGpuSet() {
            ...
            if (_adlHelper.ATIGpus.Count > 0) {
                ...
                const ulong minG = 5 * LuckyKeyword.ULongG;
                bool has470 = _gpus.Any(a => a.Key != LuckyContext.GpuAllId && a.Value.TotalMemory < minG);
                if (has470) {
        ...
        public void LoadGpuState() {
            ...
            var elapsedMilliseconds = NTStopwatch.Stop();
            if (elapsedMilliseconds.ElapsedMilliseconds > NTStopwatch.ElapsedMilliseconds) {
                LuckyConsole.DevTimeSpan($"耗时{elapsedMilliseconds} {this.GetType().Name}.{nameof(LoadGpuState)}");
            }
#endif
        }
        ...
        public bool IsLowDriverVersion {
            get {
                return this._driverVersion < LuckyContext.Instance.MinAmdDriverVersion;
            }
        }
