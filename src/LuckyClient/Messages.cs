...
namespace Lucky {
    ...
    public class MinerTweakActionCommand : Cmd {
        public MinerTweakActionCommand(MinerTweakActionType actionType) {
            this.ActionType = actionType;
        }

        public MinerTweakActionType ActionType { get; private set; }
    }
    ...
    [MessageType(description: "发现了Lucky或MinerMonitor新版本")]
            ...
            if (!string.IsNullOrEmpty(serverVersion) && serverVersion != EntryAssemblyInfo.CurrentVersionStr) {
                if (Version.TryParse(serverVersion, out Version v)) {
                    if (v > EntryAssemblyInfo.CurrentVersion) {
                        LuckyContext.SetServerVersion(v);
    ...
    [MessageType(description: "关闭Lucky客户端")]
    // ReSharper disable once InconsistentNaming
    public class CloseLuckyCommand : Cmd {
        public CloseLuckyCommand(string reason) {
            this.Reason = reason;
        }
    ...
    #region LuckyWallet Messages
    [MessageType(description: "添加Lucky钱包")]
    public class AddLuckyWalletCommand : AddEntityCommand<ILuckyWallet> {
        public AddLuckyWalletCommand(ILuckyWallet input) : base(input) {
        }
    }

    [MessageType(description: "更新Lucky钱包")]
    public class UpdateLuckyWalletCommand : UpdateEntityCommand<ILuckyWallet> {
        public UpdateLuckyWalletCommand(ILuckyWallet input) : base(input) {
        }
    }

    [MessageType(description: "删除Lucky钱包")]
    public class RemoveLuckyWalletCommand : RemoveEntityCommand {
        public RemoveLuckyWalletCommand(Guid entityId) : base(entityId) {
        }
    }

    [MessageType(description: "添加Lucky钱包后")]
    public class LuckyWalletAddedEvent : SourcedEvent<ILuckyWallet> {
        public LuckyWalletAddedEvent(PathId targetPathId, ILuckyWallet source) : base(targetPathId, source) {
        }
    }

    [MessageType(description: "更新Lucky钱包后")]
    public class LuckyWalletUpdatedEvent : SourcedEvent<ILuckyWallet> {
        public LuckyWalletUpdatedEvent(PathId targetPathId, ILuckyWallet source) : base(targetPathId, source) {
        }
    }

    [MessageType(description: "删除Lucky钱包后")]
    public class LuckyWalletRemovedEvent : SourcedEvent<ILuckyWallet> {
        public LuckyWalletRemovedEvent(PathId targetPathId, ILuckyWallet source) : base(targetPathId, source) {
        }
    }
    #endregion
    ...
    [MessageType(description: "Lucky钱包集初始化后")]
    public class LuckyWalletSetInitedEvent : EventBase {
        public LuckyWalletSetInitedEvent() {
        }
    }
    #endregion
    ...
    [MessageType(description: "添加或修改了视图后")]
    ...
    [MessageType(description: "删除了视图后")]
