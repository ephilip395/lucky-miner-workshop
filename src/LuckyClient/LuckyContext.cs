...
namespace Lucky
{
    public partial class LuckyContext : ILuckyContext
    {
        ...
        private LuckyContext()
        {
            ...
            VirtualRoot.BuildEventPath<AppExitEvent>($"程序退出时的{nameof(LuckyContext)}退出逻辑", LogEnum.None, typeof(LuckyContext), PathPriority.Normal,
                message =>
                {
        ...
        {
            VirtualRoot.MyLocalInfo(nameof(LuckyContext), $"Windows 会话切换, 因为{((WindowsSessionSwitchReason)e.Reason).GetDescription()}");
        }
        ...
        public void Init(Action callback)
        {
            _ = Task.Factory.StartNew(() =>
            {
                ...
                if (ClientAppType.IsMinerTweak)
                {
                    LuckyRegistry.SetWorkType(_workType);
                }
                if (isWork)
                {
                    DoInit(callback);
                ...
                {
                    // 如果是 Debug 模式且不是群控端则使用 serverDbFile 初始化
                    if (DevMode.IsDevMode && !ClientAppType.IsMinerMonitor)
                    {
                        if (!File.Exists(HomePath.ServerDbFileFullName))
                        {
                            ...
                            RpcRoot.OSSService.AliyunOSSService.GetAliyunServerLitedb(data =>
                            {
                                ...
                                {
                                    VirtualRoot.MyLocalError(
                                        nameof(LuckyContext),
                                        "server.litedb文件下载失败，这是第一次debug模式运行行运矿工，server.litedb 文件至少需要成功下载一次，请检查网络是否可用",
                                        OutEnum.Warn);
                                }
                                DoInit(callback);
                            });
                    ...
                    {
                        // 非开发模式或者群控端：每次启动都要下载新的 server json
                        ...
                        {
                            // 如果 server.json 未下载成功则不覆写本地 server.json
                            if (!string.IsNullOrEmpty(serverJson))
                            {
                                ...
                                RpcRoot.OfficialServer.AppSettingService.GetJsonFileVersionAsync(ClientAppType.AppType, HomePath.ExportServerJsonFileName, serverState =>
                                {
                                    ...
                                    AppVersionChangedEvent.PublishIfNewVersion(serverState.MinerTweakVersion);
                                    ...
                                    {
                                        LuckyConsole.UserWarn($"本机和服务器时间不同步，请调整，本地：{DateTime.Now}，服务器：{Timestamp.FromTimestamp(serverState.Time)}。此问题不影响挖矿。");
                                    }
                                });
                            }
                            else
                            {
                                ...
                                {
                                    VirtualRoot.MyLocalError(nameof(LuckyContext), "配置文件下载失败，这是第一次运行行运矿工，配置文件至少需要成功下载一次，请检查网络是否可用", OutEnum.Warn);
                                }
                                else
                                {
                                    VirtualRoot.MyLocalWarn(nameof(LuckyContext), "配置文件下载失败，使用最近一次成功下载的配置文件");
                                }
                            }
                            DoInit(callback);
                        });
                ...
                VirtualRoot.MyLocalInfo(nameof(LuckyContext), $"启动{VirtualRoot.AppName}");
            });
        }
        ...
        private void DoInit(Action callback)
        {
            ...
            CpuPackage cpuPackage = new CpuPackage(_minerProfile);
            this.CpuPackage = cpuPackage;

            // 这几个注册表内部区分挖矿端和群控端
            LuckyRegistry.SetLocation(ClientAppType.AppType, VirtualRoot.AppFileFullName);
            LuckyRegistry.SetArguments(ClientAppType.AppType, string.Join(" ", CommandLineArgs.Args));
            LuckyRegistry.SetCurrentVersion(ClientAppType.AppType, EntryAssemblyInfo.CurrentVersionStr);
            LuckyRegistry.SetCurrentVersionTag(ClientAppType.AppType, EntryAssemblyInfo.CurrentVersionTag);
        ...
        // 群控端管理作业时调用该方法切换MinerProfile上下文
            ...
            RpcRoot.OfficialServer.AppSettingService.GetJsonFileVersionAsync(ClientAppType.AppType, HomePath.ExportServerJsonFileName, serverState =>
            {
                ...
                if (!string.IsNullOrEmpty(serverState.JsonFileVersion) && localServerJsonFileVersion != serverState.JsonFileVersion)
                {
                    RpcRoot.OSSService.AliyunOSSService.GetAliyunServerJson((serverJson) =>
                    {
                        ...
                        SetServerJsonVersion(serverState.JsonFileVersion);
                        ContextReInit();
                        VirtualRoot.MyLocalInfo(nameof(LuckyContext), $"刷新server.json配置", toConsole: true);
                    });
                }
                else
                {
                    LuckyConsole.DevDebug("server.json没有新版本");
                }
        ...
        public string GetServerJsonVersion()
        {
            ...
            if (VirtualRoot.LocalAppSettingSet.TryGetAppSetting(LuckyKeyword.ServerJsonVersionAppSettingKey, out IAppSetting setting) && setting.Value != null)
            {
        ...
        private void SetServerJsonVersion(string serverJsonVersion)
        {
            ...
            {
                Key = LuckyKeyword.ServerJsonVersionAppSettingKey,
        ...
        private void Link()
        {
            VirtualRoot.BuildCmdPath<RegCmdHereCommand>(location: this.GetType(), LogEnum.DevConsole, path: message =>
            {
                ...
                {
                    Cmd.RegCmdHere();
                    VirtualRoot.MyLocalInfo(nameof(LuckyContext), "添加windows右键命令行成功");
                }
                catch (Exception e)
                {
                    Logger.ErrorDebugLine(e);
                    VirtualRoot.MyLocalError(nameof(LuckyContext), "添加windows右键命令行失败", OutEnum.Warn);
                }
            });
            VirtualRoot.BuildCmdPath<UnRegCmdHereCommand>(location: this.GetType(), LogEnum.DevConsole, path: message =>
            {
                ...
                {
                    Cmd.UnRegCmdHere();
                    VirtualRoot.MyLocalInfo(nameof(LuckyContext), "移除windows右键命令行成功");
                }
                catch (Exception e)
                {
                    Logger.ErrorDebugLine(e);
                    VirtualRoot.MyLocalError(nameof(LuckyContext), "移除windows右键命令行失败", OutEnum.Warn);
                }
            });
                ...
                path: message =>
                {
                    ...
                    if (IsMining && LockedMineContext.ProcessCreatedOn != DateTime.MinValue)
                    {
                        ...
                        if (coinProfile.IsLowSpeedRestartComputer && coinProfile.LowSpeed != 0 && coinProfile.LowSpeedRestartComputerMinutes > 0)
                        {
                            ...
                            if (highSpeedOn.AddMinutes(coinProfile.LowSpeedRestartComputerMinutes) < message.BornOn)
                            {
                                ...
                                VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"{coinCode}总算力持续{coinProfile.LowSpeedRestartComputerMinutes}分钟低于{coinProfile.LowSpeed}重启电脑", toConsole: true);
                    ...
                    if (IsMining && LockedMineContext.ProcessCreatedOn != DateTime.MinValue)
                    {
                        ...
                        if (coinProfile.IsLowSpeedReOverClock && coinProfile.OverClockLowSpeed != 0 && coinProfile.LowSpeedReOverClockMinutes > 0)
                        {
                            ...
                            if (overClockHighSpeedOn.AddMinutes(coinProfile.LowSpeedReOverClockMinutes) < message.BornOn)
                            {
                                ...
                                VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"{coinCode}总算力持续{coinProfile.LowSpeedReOverClockMinutes}分钟低于{coinProfile.OverClockLowSpeed}重新应用超频", toConsole: true);
                    ...
                    try
                    {
                        if (MinerProfile.IsPeriodicRestartComputer)
                        {
                            if ((DateTime.Now - this.CreatedOn).TotalMinutes > 60 * MinerProfile.PeriodicRestartComputerHours + MinerProfile.PeriodicRestartComputerMinutes)
                            {
                                ...
                                VirtualRoot.MyLocalWarn(nameof(LuckyContext), content, toConsole: true);
                                VirtualRoot.Execute(new ShowRestartWindowsCommand(countDownSeconds: 10));
                                if (!MinerProfile.IsAutoBoot || !MinerProfile.IsAutoStart)
                                {
                                    VirtualRoot.Execute(new SetAutoStartCommand(true, true));
                    ...
                    try
                    {
                        if (IsMining && MinerProfile.IsPeriodicRestartKernel && LockedMineContext.MineStartedOn != DateTime.MinValue)
                        {
                            ...
                            {
                                VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"每运行{MinerProfile.PeriodicRestartKernelHours.ToString()}小时{MinerProfile.PeriodicRestartKernelMinutes.ToString()}分钟重启内核", toConsole: true);
                                RestartMine();
                                return;// 退出
                            }
                        }
                        ...
                        if (IsMining && this.LockedMineContext.MainCoin != null)
                        {
                            ...
                            if (restartComputer || restartKernel)
                            {
                                ...
                                if (shareCount == totalShare)
                                {
                                    ...
                                    {
                                        VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"{MinerProfile.NoShareRestartKernelMinutes.ToString()}分钟无份额重启内核", toConsole: true);
                                        RestartMine();
                                        return;// 退出
                                    }
                                }
        ...
        private void StopMine(StopMineReason stopReason)
        {
            ...
            try
            {
                VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"挖矿停止。原因：{stopReason.GetDescription()}", toConsole: true);
        ...
        public void RestartMine(WorkType workType = WorkType.None, string workerName = null)
        {
            ...
            LuckyRegistry.SetWorkType(workType);
            ...
            try
            {
                ...
                _ = Task.Factory.StartNew(() =>
                {
                    ...
                    if (IsMining && isRestart)
                    {
                        ...
                        VirtualRoot.MyLocalInfo(nameof(LuckyContext), "重启内核", toConsole: true);
                    }
                    else
                    {
                        ...
                        if (IsMining)
                        {
                            ...
                            StopMine(StopMineReason.InStartMine);
                        }
        ...
        public void StartMine(Action<IKernel> callback)
        {
            ...
            {
                VirtualRoot.MyLocalInfo(nameof(LuckyContext), kernel.GetFullName() + "本地内核包不存在，开始自动下载", toConsole: true);
        ...
        private void ContinueStartMine(IKernel kernel)
        {
            ...
            VirtualRoot.MyLocalInfo(nameof(LuckyContext), "开始挖矿", toConsole: true);
            if (LockedMineContext.UseDevices.Length < GpuSet.Count)
            {
                VirtualRoot.MyLocalWarn(nameof(LuckyContext), "未启用全部显卡挖矿", toConsole: true);
            }
        }
        #endregion
                            ...
                            if (ClientAppType.IsMinerTweak)
                            {
                                try
                                {
                                    if (IsNCard)
                                    {
                                        ...
                                        {
                                            VirtualRoot.MyLocalWarn(this.GetType().Name, "检测到本机除了N卡外还插有A卡，开源不建议N卡A卡混插。", toConsole: true);
                                        }
                                    }
                                    else
                                    {
                ...
                path: message =>
                {
                    ...
                    if (_minerProfile.IsAutoStopByGpu)
                    {
                        if (IsMining)
                        {
                            ...
                            {
                                this.GpuSet.LowTemperatureOn = message.BornOn;
                                VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"自动停止挖矿，因为 GPU{maxGpu.Index} 温度连续{_minerProfile.GpuGETemperatureSeconds.ToString()}秒不低于{_minerProfile.GpuStopTemperature.ToString()}℃", toConsole: true);
                        ...
                        else
                        {
                            ...
                            if (_minerProfile.IsAutoStartByGpu && StopReason == StopMineReason.HighGpuTemperature)
                            {
                                ...
                                {
                                    this.GpuSet.HighTemperatureOn = message.BornOn;
                                    VirtualRoot.MyLocalWarn(nameof(LuckyContext), $"自动开始挖矿，因为 GPU 温度连续{_minerProfile.GpuLETemperatureSeconds.ToString()}秒不高于{_minerProfile.GpuStartTemperature.ToString()}℃", toConsole: true);
                                    StartMine();
                                }
                            }
                        }
        ...
        public Version MinAmdDriverVersion
        {
            get
            {
                return ServerContext.SysDicItemSet.TryGetDicItemValue(LuckyKeyword.ThisSystemSysDicCode, "MinAmdDriverVersion", new Version(17, 10, 2, 0));
            }
        }
        ...
        public Version MinNvidiaDriverVersion
        {
            get
            {
                return ServerContext.SysDicItemSet.TryGetDicItemValue(LuckyKeyword.ThisSystemSysDicCode, "MinNvidiaDriverVersion", new Version(399, 24));
            }
        }
