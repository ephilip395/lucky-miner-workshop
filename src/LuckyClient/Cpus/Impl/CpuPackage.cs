                            ...
                            Task.Factory.StartNew(() => {
                                ...
                                if (_minerProfile.IsAutoStopByCpu) {
                                    if (LuckyContext.Instance.IsMining) {
                                        ...
                                        if ((message.BornOn - LowTemperatureOn).TotalSeconds >= _minerProfile.CpuGETemperatureSeconds) {
                                            LowTemperatureOn = message.BornOn;
                                            VirtualRoot.MyLocalWarn(nameof(CpuPackage), $"自动停止挖矿，因为 CPU 温度连续{_minerProfile.CpuGETemperatureSeconds.ToString()}秒不低于{_minerProfile.CpuStopTemperature.ToString()}℃", toConsole: true);
                                            LuckyContext.Instance.StopMineAsync(StopMineReason.HighCpuTemperature);
                                        }
                                    }
                                    else {
                                        ...
                                        LowTemperatureOn = message.BornOn;
                                        if (_minerProfile.IsAutoStartByCpu && LuckyContext.Instance.StopReason == StopMineReason.HighCpuTemperature) {
                                            ...
                                            if ((message.BornOn - HighTemperatureOn).TotalSeconds >= _minerProfile.CpuLETemperatureSeconds) {
                                                HighTemperatureOn = message.BornOn;
                                                VirtualRoot.MyLocalWarn(nameof(CpuPackage), $"自动开始挖矿，因为 CPU 温度连续{_minerProfile.CpuLETemperatureSeconds.ToString()}秒不高于{_minerProfile.CpuStartTemperature.ToString()}℃", toConsole: true);
                                ...
                                if (_minerProfile.IsRaiseHighCpuEvent) {
                                    ...
                                    if ((message.BornOn - LowPerformanceOn).TotalSeconds >= _minerProfile.HighCpuSeconds) {
                                        LowPerformanceOn = message.BornOn;
                                        VirtualRoot.MyLocalWarn(nameof(CpuPackage), $"CPU使用率过高：连续{_minerProfile.HighCpuSeconds.ToString()}秒不低于{_minerProfile.HighCpuBaseline.ToString()}%，如果有杀毒软件需关闭或卸载杀毒软件。");
                                    }
                                }
                            });
                        });
