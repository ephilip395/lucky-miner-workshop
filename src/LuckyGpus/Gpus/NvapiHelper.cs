    ...
    public class NvapiHelper : IGpuHelper {
            ...
            get {
                ...
                lock (_locker) {
                    ...
                    if (r != NvStatus.NVAPI_OK) {
                        LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvEnumPhysicalGPUs)} {r.ToString()}");
                    }
                    for (int i = 0; i < gpuCount; i++) {
                        r = NvapiNativeMethods.NvGetBusID(handles[i], out int busId);
                        if (r != NvStatus.NVAPI_OK) {
                            LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvGetBusID)} {r.ToString()}");
                        }
                    ...
                    if (r != NvStatus.NVAPI_OK) {
                        LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvEnumTCCPhysicalGPUs)} {r.ToString()}");
                    }
                    for (int i = 0; i < gpuCount; i++) {
                        r = NvapiNativeMethods.NvGetBusID(handles[i], out int busId);
                        if (r != NvStatus.NVAPI_OK) {
...
#if DEBUG
                LuckyConsole.DevWarn(() => $"GetClockRange {result.ToString()}");
#endif
            }
            catch (Exception e) {
        ...
        public void GetTemperature(IGpu gpu, out uint coreTemperature, out uint memoryTemperature) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    settings.Count = 0;
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvGetThermalSettings)} {r.ToString()}");
                }
                if (settings.Count > 0) {
        ...
        public void SetFanSpeed(IGpu gpu, int fanSpeed) {
            ...
            if (NvapiNativeMethods.NvSetCoolerLevels != null) {
                try {
                    ...
                    if (r != NvStatus.NVAPI_OK) {
                        LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvSetCoolerLevels)} {r.ToString()}");
                    }
                    else {
                        return;
                    }
            ...
            try {
                if (NvFanCoolersGetControl(busId, out PrivateFanCoolersControlV1 info)) {
                    ...
                    if (r != NvStatus.NVAPI_OK) {
                        LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvFanCoolersSetControl)} {r.ToString()}");
                        return;
                    }
                    return;
                }
        ...
        private static void SetCoreClock(int busId, int mHz) {
            ...
            try {
                if (NvGetPStateV2(busId, out NvGpuPerfPStates20InfoV2 info)) {
                    ...
                    if (!r) {
                        LuckyConsole.DevError(() => $"{nameof(SetCoreClock)} {r.ToString()}");
                    }
                }
            }
            catch {
        ...
        private static void SetMemoryClock(int busId, int mHz) {
            ...
            try {
                if (NvGetPStateV2(busId, out NvGpuPerfPStates20InfoV2 info)) {
                    ...
                    var r = NvSetPStateV2(busId, ref info);
                    if (!r) {
                        LuckyConsole.DevError(() => $"{nameof(SetMemoryClock)} {r.ToString()}");
                    }
                }
            }
            catch {
        ...
        private static bool NvPowerPoliciesSetStatus(int busId, ref NvGpuPowerStatus info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvPowerPoliciesSetStatus)} {r.ToString()}");
                }
                if (r == NvStatus.NVAPI_OK) {
                    return true;
                }
        ...
        private static int GetVoltage(int busId) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvGetClockBoostLock)} {r.ToString()}");
                }
                if (r == NvStatus.NVAPI_OK) {
        ...
        private static void SetVoltage(int busId, int voltage) {
            ...
            try {
                ...
                if (r == NvStatus.NVAPI_OK) {
                    ...
                    if (r != NvStatus.NVAPI_OK) {
                        LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvSetClockBoostLock)} {r.ToString()}");
                    }
                }
            }
            catch {
        ...
        private static bool NvGetPStateV2(int busId, out NvGpuPerfPStates20InfoV2 info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvGetPStateV2)} {r.ToString()}");
                    return false;
                }
                return true;
            }
        ...
        private static bool NvSetPStateV2(int busId, ref NvGpuPerfPStates20InfoV2 info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvSetPStateV2)} {r.ToString()}");
                }
                if (r == NvStatus.NVAPI_OK) {
                    return true;
                }
        ...
        private static bool NvThermalPoliciesGetInfo(int busId, out NvGpuThermalInfo info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvThermalPoliciesGetInfo)} {r.ToString()}");
                    return false;
                }
                return true;
            }
        ...
        private static NvGpuThermalLimit NvThermalPoliciesGetLimit(int busId) { 
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvThermalPoliciesGetLimit)} {r.ToString()}");
                }
                if (r == NvStatus.NVAPI_OK) {
                    return info;
                }
        ...
        private static bool NvThermalPoliciesSetLimit(int busId, ref NvGpuThermalLimit info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvThermalPoliciesSetLimit)} {r.ToString()}");
                }
                if (r == NvStatus.NVAPI_OK) {
                    return true;
                }
        ...
        private static bool NvPowerPoliciesGetStatus(int busId, out NvGpuPowerStatus info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvPowerPoliciesGetStatus)} {r.ToString()}");
                    return false;
                }
                return true;
            }
        ...
        private static bool NvPowerPoliciesGetInfo(int busId, out NvGpuPowerInfo info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvPowerPoliciesGetInfo)} {r.ToString()}");
                    return false;
                }
                return true;
            }
        ...
        private static bool GetFanCoolersGetStatus(int busId, out PrivateFanCoolersStatusV1 info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    ...
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvFanCoolersGetStatus)} {r.ToString()}");
                    return false;
                }
                return true;
            }
        ...
        private static bool GetCoolerSettings(int busId, out NvCoolerSettings info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    ...
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvGetCoolerSettings)} {r.ToString()}");
                    return false;
                }
                return true;
            }
        ...
        private static bool NvFanCoolersGetControl(int busId, out PrivateFanCoolersControlV1 info) {
            ...
            try {
                ...
                if (r != NvStatus.NVAPI_OK) {
                    LuckyConsole.DevError(() => $"{nameof(NvapiNativeMethods.NvFanCoolersGetControl)} {r.ToString()}");
                    return false;
                }
                return true;
            }
