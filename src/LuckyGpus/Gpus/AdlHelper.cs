    ...
    public class AdlHelper : IGpuHelper {
        ...
        static AdlHelper() {
            try {
                ...
                if (adlStatus >= AdlStatus.ADL_OK) {
                    ...
                    if (adlStatus < AdlStatus.ADL_OK) {
                        LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL_Adapter_NumberOfAdapters_Get)} {adlStatus.ToString()}");
                    }
                    LuckyConsole.DevDebug(() => $"{nameof(numberOfAdapters)}={numberOfAdapters.ToString()}");
                }
                if (numberOfAdapters > 0) {
                    ...
                    foreach (var adapterInfo in _adapterInfoes) {
                        LuckyConsole.DevDebug(() => adapterInfo.ToString());
                    }
                }
            }
            catch (Exception ex) {
                    ...
                    foreach (var adapterInfo in _adapterInfoes) {
                        ...
                        if (!gpuInited) {
                            ...
                            try {
                                ...
                                LuckyConsole.DevDebug(() => "overdriveVersion=" + overdriveVersion.ToString());
                            }
                            catch (Exception ex) {
                                Logger.ErrorDebugLine(ex);
                            }
                            ...
                            try {
                                ...
                                if (r < AdlStatus.ADL_OK) {
                                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_CapabilitiesX2_Get)} {r.ToString()}");
                                }
                                else {
                                    LuckyConsole.DevDebug(() => "aDLODNCapabilitiesX2=" + aDLODNCapabilitiesX2.ToString());
                                }
                            }
                            catch (Exception ex) {
                                Logger.ErrorDebugLine(ex);
                            ...
                            try {
                                ...
                                if (r < AdlStatus.ADL_OK) {
                                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_SystemClocksX2_Get)} {r.ToString()}");
                                }
                                else {
                                    LuckyConsole.DevDebug(() => "systemClockX2=" + VirtualRoot.JsonSerializer.Serialize(systemClockX2));
                                }
                            }
                            catch (Exception ex) {
                                Logger.ErrorDebugLine(ex);
                            ...
                            try {
                                ...
                                if (r < AdlStatus.ADL_OK) {
                                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_MemoryClocksX2_Get)} {r.ToString()}");
                                }
                                else {
                                    LuckyConsole.DevDebug(() => "memoryClockX2=" + VirtualRoot.JsonSerializer.Serialize(memoryClockX2));
                                }
                            }
                            catch (Exception ex) {
                                Logger.ErrorDebugLine(ex);
                            ...
                            int[] memoryTimingLevels = GetMemoryTimingLevels(adapterIndex, out int currentMemoryTimingLevel, out int defaultMemoryTimingLevel);
                            _atiGpus.Add(new ATIGPU {
                                ...
                                MemoryTimingLevels = memoryTimingLevels,
                                CurrentMemoryTimingLevel = currentMemoryTimingLevel,
                                DefaultMemoryTimingLevel = defaultMemoryTimingLevel
                            });
                        }
                    }
                }
                _atiGpus = _atiGpus.OrderBy(a => a.BusNumber).ToList();
                LuckyConsole.DevDebug(() => string.Join(",", _atiGpus.Select(a => a.AdapterIndex)));
            }
            catch (Exception ex) {
                Logger.ErrorDebugLine(ex);
                return false;
        ...
        public Version GetDriverVersion() {
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Graphics_VersionsX2_Get)} {r.ToString()}");
                }
        ...
        public void OverClock(IGpu gpu, OverClockValue value) {
            ...
            try {
                ...
                if (isSetMemoryTimingLevel) {
                    SetMemoryTimingLevel(atiGpu, value.GetMemoryTimingLevel(atiGpu));
                }
                return;
            }
            catch (Exception e) {
        ...
        public OverClockRange GetClockRange(IGpu gpu) {
            ...
            try {
                ...
                result.CurrentMemoryTimingLevel = atiGpu.CurrentMemoryTimingLevel;
...
#if DEBUG
                LuckyConsole.DevDebug(() => $"GetClockRange {result.ToString()}");
#endif
            }
            catch (Exception e) {
                Logger.ErrorDebugLine(e);
        ...
        private int[] GetMemoryTimingLevels(int adapterIndex, out int currentLevel, out int defaultLevel) {
            currentLevel = -1;
            defaultLevel = -1;
            try {
                Type elementType = typeof(int);
                int elementSize = Marshal.SizeOf(elementType);
                IntPtr lppLevelList = Marshal.AllocHGlobal(AdlConst.ADL_PERFORMANCE_LEVELS * elementSize);
                var r = AdlNativeMethods.ADL2_OverdriveN_MemoryTimingLevel_Get(
                    _context, adapterIndex, out int lpSupport, out int lpCurrentValue,
                    out int lpDefaultValue, out int lpNumberLevels, ref lppLevelList);
                currentLevel = lpCurrentValue;
                defaultLevel = lpDefaultValue;
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_MemoryTimingLevel_Get)} {r.ToString()}");
                }
                if (lpNumberLevels == 0) {
                    return new int[0];
                }
                int[] levelList = new int[lpNumberLevels];
                for (int i = 0; i < levelList.Length; i++) {
                    levelList[i] = (int)Marshal.PtrToStructure((IntPtr)((long)lppLevelList + i * elementSize), elementType);
                }
                Marshal.FreeHGlobal(lppLevelList);
                return levelList;
            }
            catch (Exception e) {
                Logger.ErrorDebugLine(e);
        ...
        private void SetMemoryTimingLevel(ATIGPU atiGpu, int level) {
            try {
                if (!atiGpu.MemoryTimingLevels.Contains(level)) {
                    return;
                }
                var r = AdlNativeMethods.ADL2_OverdriveN_MemoryTimingLevel_Set(_context, atiGpu.AdapterIndex, level);
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_MemoryTimingLevel_Set)} {r.ToString()}");
                    return;
                }
                else {
                    atiGpu.CurrentMemoryTimingLevel = level;
                }
            }
            catch (Exception e) {
                Logger.ErrorDebugLine(e);
        ...
        private static void SetCoreClockOld(ATIGPU atiGpu, int value, int voltage) {
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_SystemClocksX2_Set)} {r.ToString()}");
                    return;
                }
                if (isReset) {
                    return;
                ...
                LuckyConsole.DevDebug(() => $"{nameof(SetCoreClockOld)} PState {index.ToString()} value={value.ToString()} voltage={voltage.ToString()}");
                if (value != 0) {
        ...
        private static void SetMemoryClockOld(ATIGPU atiGpu, int value, int voltage) {
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_MemoryClocksX2_Set)} {r.ToString()}");
                    return;
                }
                if (isReset) {
                    return;
                ...
                LuckyConsole.DevDebug(() => $"{nameof(SetMemoryClockOld)} PState {index.ToString()} value={value.ToString()} voltage={voltage.ToString()}");
                if (value != 0) {
                    info.aLevels[index].iClock = value * 100;
                }
                if (voltage != 0) {
            ...
            try {
                ...
                if (isAutoMode) {
                    try {
                        ...
                        if (r != AdlStatus.ADL_OK) {
                            LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Overdrive5_FanSpeedToDefault_Set)} {r.ToString()}");
                        }
                    }
                    catch (Exception e) {
                        r = AdlStatus.ADL_ERR;
                    ...
                    if (r != AdlStatus.ADL_OK) {
                        try {
                            ...
                            if (r != AdlStatus.ADL_OK) {
                                LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL_Overdrive5_FanSpeedToDefault_Set)} {r.ToString()}");
                            }
                        }
                        catch (Exception e) {
                            Logger.ErrorDebugLine(e);
                    ...
                    if (r != AdlStatus.ADL_OK) {
                        try {
                            ...
                            if (r != AdlStatus.ADL_OK) {
                                LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Overdrive6_FanSpeed_Reset)} {r.ToString()}");
                            }
                        }
                        catch (Exception e) {
                            Logger.ErrorDebugLine(e);
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL_Overdrive5_FanSpeed_Get)} {r.ToString()}");
                    return;
                }
                info.FanSpeed = value;
                r = AdlNativeMethods.ADL_Overdrive5_FanSpeed_Set(atiGpu.AdapterIndex, 0, ref info);
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL_Overdrive5_FanSpeed_Set)} {r.ToString()}");
                    return;
                }
                return;
            }
        ...
        private static void SetPowerLimitOld(ATIGPU atiGpu, int value) {
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_PowerLimit_Get)} {r.ToString()}");
                    return;
                }
                info.iMode = AdlConst.ODNControlType_Manual;
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_PowerLimit_Set)} {r.ToString()}");
                    return;
                }
                return;
            }
        ...
        public void GetPowerFanTemp(int gpuIndex, out uint power, out uint fanSpeed, out int coreTemp, out int memTemp) {
            power = 0;
            fanSpeed = 0;
            coreTemp = 0;
            memTemp = 0;
            ...
            try {
                if (atiGpu.OverdriveVersion < 8) {
                    coreTemp = GetTemperatureOld(atiGpu);
                    ...
                    if (r < AdlStatus.ADL_OK) {
                        LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_New_QueryPMLogData_Get)} {r.ToString()}");
                    }
                    ...
                    if (i < logDataOutput.Sensors.Length && logDataOutput.Sensors[i].Supported != 0) {
                        coreTemp = logDataOutput.Sensors[i].Value;
                    }
                    i = (int)ADLSensorType.PMLOG_TEMPERATURE_MEM;
                    if (i < logDataOutput.Sensors.Length && logDataOutput.Sensors[i].Supported != 0) {
                        memTemp = logDataOutput.Sensors[i].Value;
                    }
                }
            }
            catch {
        ...
        public ulong GetTotalMemory(int gpuIndex) {
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL_Adapter_MemoryInfo_Get)} {r.ToString()}");
                    return 0;
                }
        ...
        private static int GetTemperatureOld(ATIGPU atiGpu) {
            try {
                if (atiGpu.OverdriveVersion >= 7) {
                    ...
                    if (r < AdlStatus.ADL_OK) {
                        LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_OverdriveN_Temperature_Get)} {r.ToString()}");
                        return 0;
                    }
                    ...
                    if (r < AdlStatus.ADL_OK) {
                        LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL_Overdrive5_Temperature_Get)} {r.ToString()}");
                        return 0;
                    }
        ...
        private static uint GetPowerUsageOld(ATIGPU atiGpu) {
            try {
                ...
                if (atiGpu.OverdriveVersion >= 6) {
                    ...
                    if (r < AdlStatus.ADL_OK) {
                        LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Overdrive6_CurrentPower_Get)} {r.ToString()}");
                        return 0;
                    }
        ...
        private static bool GetOD8InitSetting(int adapterIndex, out ADLOD8InitSetting aDLOD8InitSetting) {
            ...
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Overdrive8_Init_Setting_Get)} {r.ToString()}");
                }
#if DEBUG
        ...
        private static bool GetOD8CurrentSetting(int adapterIndex, out ADLOD8CurrentSetting aDLOD8CurrentSetting) {
            ...
            try {
                ...
                if (r < AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Overdrive8_Current_Setting_Get)} {r.ToString()}");
                }
#if DEBUG
        ...
        private static bool SetOD8Range(
            ...
            try {
                ...
                if (r != AdlStatus.ADL_OK) {
                    LuckyConsole.DevError(() => $"{nameof(AdlNativeMethods.ADL2_Overdrive8_Setting_Set)} {r.ToString()}");
                }
                return r == AdlStatus.ADL_OK;
            }
            catch (Exception e) {
