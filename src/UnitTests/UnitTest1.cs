...
namespace Lucky {
    ...
    public class UnitTest1 {
        ...
        public void UriPathTest() {
            string url = $"http://{LuckyKeyword.CloudFileDomain}/release/LuckyUpdater.exe?Expires=1583773008";
            Uri uri = new Uri(url);
            Assert.AreEqual("/LuckyUpdater.exe", uri.LocalPath);
            Assert.AreEqual("LuckyUpdater.exe", Path.GetFileName(uri.LocalPath));
        }
...
#pragma warning restore IDE0028 // 简化集合初始化
        ...
        public void GetIndexCharTest() {
            ...
            for (int i = 0; i < 12; i++) {
                chars1.Add(LuckyKeyword.GetIndexChar(i, string.Empty));
                chars2.Add(LuckyKeyword.GetIndexChar(i, ","));
            }
