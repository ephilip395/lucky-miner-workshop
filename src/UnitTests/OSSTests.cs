...
namespace Lucky {
    ...
    public class OSSTests {
        ...
        public void AliGeneratePresignedUriTest() {
            string ossEndpoint = LuckyKeyword.CloudFileDomain;
            ...
            var req = new GeneratePresignedUriRequest("luckycm", "helloworld.exe", SignHttpMethod.Get) {
                Expiration = DateTime.Now.AddMinutes(10)
            };
            var uri = ossClient.GeneratePresignedUri(req);
            Console.WriteLine(uri.ToString());
            req = new GeneratePresignedUriRequest("luckycm", "helloworld.exe", SignHttpMethod.Get);
