...
/// 嵌入的资源的位置和命名空间有关契约关系
/// </summary>
namespace Lucky.OhGodAnETHlargementPill {
    public static class OhGodAnETHlargementPillUtil {
        ...
        public static void Start() {
            try {
                if (!LuckyContext.Instance.MinerProfile.Is1080PillEnabled) {
                    return;
                }
                if (LuckyContext.Instance.GpuSet.AsEnumerable().Any(a => a.Name.IndexOf("1080", StringComparison.OrdinalIgnoreCase) != -1)) {
                    ExtractResource();
