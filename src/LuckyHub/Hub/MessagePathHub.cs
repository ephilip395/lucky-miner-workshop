namespace Lucky.Hub
{
    ...
    // Dictionary<type(Message), List<MessagePath>>
    // MessagePath<Message>
    public class MessagePathHub : IMessagePathHub
    {

        // isEnnable 和 viaTimesLimmit 改变时发出PropertyChangedEvent
        private static bool _enableNotifyProperty = false;
        /// <summary>
        /// 启用通知机制从而使展示MessagePath集合的界面响应MessagePath对象状态的变化。
        /// </summary>
        public static void EnableNotifyProperty()
        {
            _enableNotifyProperty = true;
        }
        ...
        public void Route<TMessage>(TMessage message) where TMessage : IMessage
        {
            ...
            if (messagePaths.Length == 0)
            {
                ...
                {
                    LuckyConsole.DevWarn(() => messageType.FullName + "类型的消息没有对应的处理器");
                }
            }
            else
            {
                        ...
                        if (canGo)
                        {
                            switch (messagePath.LogType)
                            {
                                case LogEnum.DevConsole:
                                    ...
                                    {
                                        LuckyConsole.DevDebug(() => $"({typeof(TMessage).Name})->({messagePath.Location}){messagePath.Description}");
                                    }
                                    break;
                                case LogEnum.UserConsole:
                                    LuckyConsole.UserInfo($"({typeof(TMessage).Name})->({messagePath.Location}){messagePath.Description}");
                                    break;
                                case LogEnum.Log:
        ...
        private class MessagePathSet<TMessage> : IMessagePathSet
        {
            ...
            public void AddMessagePath(MessagePath<TMessage> messagePath)
            {
                lock (_locker)
                {
                    ...
                    {
                        // 其他类型: PathName && PathId && Priority 警告路径重复
                        ...
                        if (paths.Length != 0)
                        {
                            ...
                            {
                                LuckyConsole.DevWarn(() => $"重复的路径:{path.PathName} {path.Description}");
                            }
                        }
                    }
            ...
            public void RemoveMessagePath(IMessagePathId messagePathId)
            {
                lock (_locker)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        LuckyConsole.DevDebug(() => "拆除路径" + messagePathId.PathName + messagePathId.Description);
                    }
                }
            }
        }
