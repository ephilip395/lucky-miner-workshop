...
namespace Lucky.MinerMonitor.Vms
{
    public class ColumnsShowViewModel : ViewModelBase, IColumnsShow
    {
        ...
        public ColumnsShowViewModel(Guid id)
        {
            ...
            this.Hide = new DelegateCommandTpl<string>((propertyName) =>
            {
            ...
            this.Remove = new DelegateCommand(() =>
            {
                ...
                this.ShowSoftDialog(new DialogWindowViewModel(message: $"您确定删除 “{this.ColumnsShowName}” 视图吗？", title: "确认", onYes: () =>
                {
                    LuckyContext.MinerMonitorContext.ColumnsShowSet.Remove(this.Id);
                }));
            });
        }
        ...
        private void Save()
        {
            if (_isSave)
            {
                LuckyContext.MinerMonitorContext.ColumnsShowSet.AddOrUpdate(new ColumnsShowData().Update(this));
            }
        }
        ...
        public const string VERSION = "软件版本";
        [Description(VERSION)]
        public bool Version
        {
            get => _version;
