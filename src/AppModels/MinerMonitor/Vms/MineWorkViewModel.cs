...
namespace Lucky.MinerMonitor.Vms
{
    public class MineWorkViewModel : ViewModelBase, IMineWork, IEditableViewModel
    {
        ...
        {
            if (!MinerMonitorRoot.MineWorkVms.TryGetMineWorkVm(this.Id, out MineWorkViewModel mineWorkVm) && !this.Id.IsSelfMineWorkId())
            {
            ...
            {
                _minerClientVm = MinerMonitorRoot.MinerTweaksWindowVm.SelectedMinerTweaks.FirstOrDefault();
                ...
                if (this.Id.IsSelfMineWorkId())
                {
                    ...
                    if (RpcRoot.IsOuterNet)
                    {
                        ...
                        VirtualRoot.BuildOncePath<GetSelfWorkLocalJsonResponsedEvent>("填充Vm内存", LogEnum.DevConsole, PathId.Empty, typeof(MineWorkViewModel), PathPriority.Normal, path: message =>
                        {
                        ...
                        MinerMonitorRoot.MinerMonitorService.GetSelfWorkLocalJsonAsync(_minerClientVm);
                    }
                    else
                    {
                        RpcRoot.Client.LuckyDaemonService.GetSelfWorkLocalJsonAsync(_minerClientVm, (json, e) =>
                        {
                            string data = json;
                            EditJson(formType, WorkType.SelfWork, data);
                        });
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            string localJsonFileFullName = MinerMonitorPath.GetMineWorkLocalJsonFileFullName(this.Id);
        ...
        private void EditJson(FormType? formType, WorkType workType, string json)
        {
            ...
            LuckyContext.Instance.ReInitMinerProfile(workType);
            this.Sha1 = LuckyContext.Instance.MinerProfile.GetSha1();
            MineWorkData mineWorkData = new MineWorkData().Update(this);
            LocalJsonDb localJsonObj = new LocalJsonDb(LuckyContext.Instance, mineWorkData);
            ServerJsonDb serverJsonObj = new ServerJsonDb(LuckyContext.Instance, localJsonObj);
        ...
        private void DoSave()
        {
            ...
            MineWorkData mineWorkData = new MineWorkData().Update(this);
            if (MinerMonitorRoot.MineWorkVms.TryGetMineWorkVm(this.Id, out MineWorkViewModel vm))
            {
                string sha1 = LuckyContext.Instance.MinerProfile.GetSha1();
                // 如果作业设置变更了则一定变更了
                if (this.Sha1 != sha1)
                {
            ...
            if (RpcRoot.IsOuterNet)
            {
                if (!this.Id.IsSelfMineWorkId())
                {
                    RpcRoot.OfficialServer.UserMineWorkService.AddOrUpdateMineWorkAsync(new MineWorkData().Update(this), (r, ex) =>
                    {
                        if (r.IsSuccess())
                        {
                            ...
                            {
                                LuckyContext.Instance.ExportWorkJson(mineWorkData, out string localJson, out string serverJson);
                ...
                else
                {
                    if (isMinerProfileChanged)
                    {
                        ...
                        if (!string.IsNullOrEmpty(localJson) && !string.IsNullOrEmpty(serverJson))
                        {
                            MinerMonitorRoot.MinerMonitorService.SaveSelfWorkLocalJsonAsync(_minerClientVm, localJson, serverJson);
                        }
                        if (mineWorkData.ServerJsonSha1 != this.ServerJsonSha1)
                        {
                            this.ServerJsonSha1 = mineWorkData.ServerJsonSha1;
                ...
                if (!this.Id.IsSelfMineWorkId())
                {
                    if (isMinerProfileChanged)
                    {
                        ...
                        if (!string.IsNullOrEmpty(localJson) && !string.IsNullOrEmpty(serverJson))
                        {
                            ...
                            {
                                string localJsonFileFullName = MinerMonitorPath.GetMineWorkLocalJsonFileFullName(this.Id);
                                string serverJsonFileFullName = MinerMonitorPath.GetMineWorkServerJsonFileFullName(this.Id);
                    ...
                    if (LuckyContext.MinerMonitorContext.MineWorkSet.Contains(mineWorkData.Id))
                    {
        ...
        public string Name
        {
            ...
            set
            {
                if (_name != value)
                {
                    ...
                    if (MinerMonitorRoot.MineWorkVms.List.Any(a => a.Name == value && a.Id != this.Id))
                    {
