...
namespace Lucky {
    public static partial class AppRoot {
        public class SysDicItemViewModels : ViewModelBase {
            ...
            private void Init() {
                foreach (var item in LuckyContext.Instance.ServerContext.SysDicItemSet.AsEnumerable().ToArray()) {
            ...
            public List<SysDicItemViewModel> KernelBrandItems {
                get {
                    ...
                    if (SysDicVms.TryGetSysDicVm(LuckyKeyword.KernelBrandSysDicCode, out SysDicViewModel sysDic)) {
            ...
            public List<SysDicItemViewModel> PoolBrandItems {
                get {
                    List<SysDicItemViewModel> list = new List<SysDicItemViewModel>();
                    if (SysDicVms.TryGetSysDicVm(LuckyKeyword.PoolBrandSysDicCode, out SysDicViewModel sysDic)) {
                        list.AddRange(List.Where(a => a.DicId == sysDic.Id).OrderBy(a => a.SortNumber));
                    }
                    return list;
                }
            ...
            public List<SysDicItemViewModel> AlgoItems {
                get {
                    List<SysDicItemViewModel> list = new List<SysDicItemViewModel>();
                    if (SysDicVms.TryGetSysDicVm(LuckyKeyword.AlgoSysDicCode, out SysDicViewModel sysDic)) {
                        list.AddRange(List.Where(a => a.DicId == sysDic.Id).OrderBy(a => a.SortNumber));
                    }
                    return list;
                }
