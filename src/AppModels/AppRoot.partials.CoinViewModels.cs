...
namespace Lucky
{
    public static partial class AppRoot
    {
        public class CoinViewModels : ViewModelBase
        {
                ...
                BuildEventPath<CoinIconDownloadedEvent>("刷新图标", LogEnum.DevConsole, location: this.GetType(), PathPriority.Normal,
                    path: message =>
                    {
                        try
                        {
                            ...
                            string iconFileFullName = MinerTweakTempPath.GetIconFileFullName(message.Source.Icon);
            ...
            private void Init()
            {
                foreach (var item in LuckyContext.Instance.ServerContext.CoinSet.AsEnumerable().ToArray())
                {
            ...
            {
                if (LuckyContext.Instance.ServerContext.CoinSet.TryGetCoin(coinCode, out ICoin coin))
                {
