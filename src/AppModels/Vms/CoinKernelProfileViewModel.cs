    ...
    public class CoinKernelProfileViewModel : ViewModelBase, ICoinKernelProfile {
        ...
        public Guid CoinKernelId {
            ...
            set {
                if (_inner.CoinKernelId != value) {
                    LuckyContext.Instance.MinerProfile.SetCoinKernelProfileProperty(this.CoinKernelId, nameof(CoinKernelId), value);
        ...
        public bool IsDualCoinEnabled {
            ...
            set {
                if (_inner.IsDualCoinEnabled != value) {
                    LuckyContext.Instance.MinerProfile.SetCoinKernelProfileProperty(this.CoinKernelId, nameof(IsDualCoinEnabled), value);
                    OnPropertyChanged(nameof(IsDualCoinEnabled));
                    // 不必判断该对象是否是主界面上当前展示的对象，因为若不是主界面上当前显式的对象的话没有机会变更
                    LuckyContext.RefreshArgsAssembly.Invoke("币种内核Profile上放置的挖矿双挖启用状态发生了变更");
                }
            }
        }
        public Guid DualCoinId {
            ...
            set {
                if (_inner.DualCoinId != value) {
                    LuckyContext.Instance.MinerProfile.SetCoinKernelProfileProperty(this.CoinKernelId, nameof(DualCoinId), value);
        ...
        public bool IsAutoDualWeight {
            get {
                ...
                if (LuckyContext.Instance.ServerContext.CoinKernelSet.TryGetCoinKernel(this.CoinKernelId, out coinKernel)) {
                    IKernel kernel;
                    if (LuckyContext.Instance.ServerContext.KernelSet.TryGetKernel(coinKernel.KernelId, out kernel)) {
                        IKernelInput kernelInput;
                        if (LuckyContext.Instance.ServerContext.KernelInputSet.TryGetKernelInput(kernel.KernelInputId, out kernelInput)) {
            ...
            set {
                if (_inner.IsAutoDualWeight != value) {
                    LuckyContext.Instance.MinerProfile.SetCoinKernelProfileProperty(this.CoinKernelId, nameof(IsAutoDualWeight), value);
                    OnPropertyChanged(nameof(IsAutoDualWeight));
                    // 不必判断该对象是否是主界面上当前展示的对象，因为若不是主界面上当前显式的对象的话没有机会变更
                    LuckyContext.RefreshArgsAssembly.Invoke("币种内核Profile上放置的挖矿双挖权重发生了变更");
                }
            }
        }
            ...
            set {
                if (_inner.CustomArgs != value) {
                    if (AppRoot.CoinKernelVms.TryGetCoinKernelVm(this.CoinKernelId, out CoinKernelViewModel coinKernelVm)) {
                        LuckyContext.Instance.MinerProfile.SetCoinKernelProfileProperty(this.CoinKernelId, nameof(CustomArgs), value);
                        OnPropertyChanged(nameof(CustomArgs));
                        // 不必判断该对象是否是主界面上当前展示的对象，因为若不是主界面上当前显式的对象的话没有机会变更
                        LuckyContext.RefreshArgsAssembly.Invoke("币种内核Profile上放置的用户自定义挖矿参数发生了变更");
                        ...
                        foreach (var gpuVm in AppRoot.GpuVms.Items) {
                            if (gpuVm.Index == LuckyContext.GpuAllId) {
                                continue;
                            }
        ...
        public string TouchedArgs {
            ...
            set {
                LuckyContext.Instance.MinerProfile.SetCoinKernelProfileProperty(this.CoinKernelId, nameof(TouchedArgs), value);
        ...
        public CoinViewModel SelectedDualCoin {
            ...
            set {
                if (value != null && value.Id != Guid.Empty) {
                    ...
                    // 不必判断该对象是否是主界面上当前展示的对象，因为若不是主界面上当前显式的对象的话没有机会变更
                    LuckyContext.RefreshArgsAssembly.Invoke("币种内核Profile上放置的挖矿选用的双挖币种发生了变更");
                }
            }
        }
    }
