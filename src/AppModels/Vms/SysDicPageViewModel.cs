        ...
        public SysDicPageViewModel() {
            ...
            this.ClearAlgo = new DelegateCommand(() => {
                this.ShowSoftDialog(new DialogWindowViewModel(message: $"确定清理未使用的算法字典项吗？", title: "确认", onYes: () => {
                    var items = LuckyContext.Instance.ServerContext.SysDicItemSet.GetSysDicItems("Algo");
                    if (items != null) {
                        var toClearItemIds = items.Where(a => LuckyContext.Instance.ServerContext.CoinSet.AsEnumerable().All(c => c.AlgoId != a.GetId())).Select(a => a.GetId()).ToArray();
