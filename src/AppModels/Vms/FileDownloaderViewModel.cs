...
namespace Lucky.Vms {
    public class FileDownloaderViewModel : ViewModelBase {
        ...
        private void Download(Action<bool, string, string> downloadComplete) {
            ...
            string saveFileFullName = Path.Combine(MinerTweakTempPath.DownloadDirFullName, Guid.NewGuid().ToString());
            using (var webClient = VirtualRoot.CreateWebClient()) {
                ...
                webClient.DownloadFileCompleted += (object sender, System.ComponentModel.AsyncCompletedEventArgs e) => {
                    ...
                    // MillisecondsDelay不依赖其它条件，因为挖矿端程序后缀upgrade=Lucky2.8.1.2.exe参数升级的时候Hub系统未就绪
