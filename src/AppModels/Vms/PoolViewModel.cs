    ...
    public class PoolViewModel : ViewModelBase, IPool, IEditableViewModel, ISortable {
        ...
        public PoolViewModel(Guid id) {
            ...
            this.Save = new DelegateCommand(() => {
                ...
                if (LuckyContext.Instance.ServerContext.PoolSet.Contains(this.Id)) {
            ...
            this.ViewPoolIncome = new DelegateCommand(() => {
                ...
                url = url.Replace("{worker}", LuckyContext.Instance.MinerProfile.MinerName);
                VirtualRoot.Execute(new UnTopmostCommand());
                Process.Start(url);
            });
        }
        ...
        public bool IsNew {
            get {
                return !LuckyContext.Instance.ServerContext.PoolSet.Contains(this.Id);
            }
        }
        ...
        private static ICoinProfile GetDualCoinProfile() {
            var workProfile = LuckyContext.Instance.MinerProfile;
        ...
        private void RefreshArgsAssembly(string reason) {
            var mainCoinProfile = LuckyContext.Instance.MinerProfile.GetCoinProfile(LuckyContext.Instance.MinerProfile.CoinId);
            if (mainCoinProfile.PoolId == this.Id || mainCoinProfile.PoolId1 == this.Id) {
                LuckyContext.RefreshArgsAssembly.Invoke(reason);
            }
            else {
