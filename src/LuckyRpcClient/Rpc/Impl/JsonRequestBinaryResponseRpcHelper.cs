            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    using (HttpClient client = RpcRoot.CreateHttpClient())
                    {
                        ...
                        {
                            callback?.Invoke(default, new LuckyHttpException($"{action} http response {getHttpResponse.Result.StatusCode.ToString()} {getHttpResponse.Result.ReasonPhrase}")
                            {
            ...
            Task.Factory.StartNew(() =>
            {
                try
                {
                    using (HttpClient client = RpcRoot.CreateHttpClient())
                    {
                        ...
                        string url = RpcRoot.GetUrl(host, port, controller, action, query);

                        Task<HttpResponseMessage> getHttpResponse = client.PostAsJsonAsync(url, data);
                        if (getHttpResponse.Result.IsSuccessStatusCode)
                        {
                            getHttpResponse.Result.Content.ReadAsByteArrayAsync().ContinueWith(t =>
                            {
