...
namespace Lucky
{
    ...
    public class BlankWindow : Window
    {
        ...
        {
            get => (bool)GetValue(HasMaximizedProperty);
        ...
        {
            bool draggable = GetDraggable((DependencyObject)e.Source) || GetDraggable((DependencyObject)e.OriginalSource);
            if (!draggable)
            {
                return;
            }
            ...
            if (e.ChangedButton == MouseButton.Left)
            {
                ...
                #endregion
                _ = UnsafeNativeMethods.SendMessage(windowHandle, Constants.WM_NCLBUTTONDOWN, Constants.HT_CAPTION, lParam);
                bool canResize = ResizeMode == ResizeMode.CanResizeWithGrip || ResizeMode == ResizeMode.CanResize;
