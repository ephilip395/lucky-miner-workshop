...
namespace Lucky
{
    public static partial class CommandLineArgs
    {
        ...
        public static string LuckyFileName { get; private set; }

        static CommandLineArgs()
        {
            IsAutoStart = Args.Contains(LuckyKeyword.AutoStartCmdParameterName, StringComparer.OrdinalIgnoreCase);
            Upgrade = PickArgument(LuckyKeyword.UpgradeCmdParameterName);
            Action = PickArgument(LuckyKeyword.ActionCmdParameterName);
            LuckyFileName = PickArgument("luckycmFileName=");
        }
